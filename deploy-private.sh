#!/usr/bin/env bash
# =============================================================================
#  Zenta — Script de despliegue EC2 PRIVADA
#  Instala Docker, clona el repo y levanta: MongoDB + Backend + Intranet
#
#  Uso:
#    chmod +x deploy-private.sh
#    sudo ./deploy-private.sh
#
...
# ── Verificar root ────────────────────────────────────────────────────────────
[[ $EUID -ne 0 ]] && error "Ejecuta con sudo: sudo ./deploy-private.sh"

info "=== Zenta — Despliegue EC2 Privada (Intranet) ==="
...
# Generado automáticamente por deploy-private.sh — $(date)
NODE_ENV=production
PORT=5000
...
echo ""
echo -e "${GREEN}Intranet disponible en:${NC} http://$(curl -s http://169.254.169.254/latest/meta-data/local-ipv4 2>/dev/null || hostname -I | awk '{print $1}')"
echo -e "${GREEN}Login:${NC} admin@zenta.mx / zenta2024"
echo ""
warn "IMPORTANTE: Cambia la contraseña de admin y los secretos JWT/EFIRMA en producción real."
